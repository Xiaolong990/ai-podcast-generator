...
📝 生成脚本 → 🔍 合规审查 → ⭐ 质量审查 → 📤 飞书审核 → ⏸️ 等待审核
                                          ...
                                          你回复"同意"后继续 ↓
                                                          ↓
                             🎨 图像 → 🎙️ 音频 → 🎬 视频 → 📊 验证 → 📋 Obsidian → 📜 LRC → 🔧 LRC修复
```
...
| 10. LRC字幕 | 生成 | 中文时间轴字幕 |
| **11. LRC修复** | fix_lrc.py | **移除英文+调整时间戳** |
...
A: 使用 v6 版本（`video_v6.py`），它基于每段音频真实时长生成 SRT，精确同步。如果LRC字幕仍有问题，运行修复脚本：
```bash
python3 /Users/duren/hermes_output/fix_lrc.py
```

### Q: LRC字幕包含英文

A: 运行修复脚本自动移除英文，只保留中文：
```bash
python3 /Users/duren/hermes_output/fix_lrc.py
```
...
11. **LRC字幕需修复**：自动生成的LRC字幕可能包含英文文本且时间戳不同步。每次生成后必须运行修复脚本：
```bash
python3 /Users/duren/hermes_output/fix_lrc.py
```
该脚本会自动移除英文、调整时间戳，确保字幕与音频完全同步。
...
- **v1.4.0** (2026-06-14): 添加LRC字幕修复步骤（步骤11）；移除英文文本，只保留中文；自动调整时间戳同步
